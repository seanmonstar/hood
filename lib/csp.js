...
const REPORT_ONLY_HEADERS = HEADERS.map(function(h) {
  return h + '-Report-Only';
});
...
// csp(policyStr, reportOnly)
function cspFactory(options, reportOnly) {
  options = options || {};
  if (isString(options)) {
    options = { policy: options, reportOnly: !!reportOnly };
  }
  var headers = options.headers ||
    (options.reportOnly && REPORT_ONLY_HEADERS) || HEADERS;
